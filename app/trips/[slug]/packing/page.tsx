...
type PackingItem = { id: string; label: string; category: string; packed: boolean };

const defaultItems: PackingItem[] = [
  { id: "documents-passport", label: "Passports and driving licences", category: "Documents", packed: false },
  { id: "documents-bookings", label: "Travel and accommodation confirmations", category: "Documents", packed: false },
  { id: "documents-insurance", label: "Travel and vehicle insurance details", category: "Documents", packed: false },
  { id: "gear-bags", label: "Luggage, day bags and waterproof covers", category: "Travel gear", packed: false },
  { id: "gear-weather", label: "Weather-appropriate outer layers", category: "Travel gear", packed: false },
  ...
  { id: "clothing-layers", label: "Clothing layers appropriate for the route", category: "Clothing", packed: false },
];
...
export default function TripPackingPage() {
  ...
  const storageKey = `two-wheels-one-way:trip:${slug}:packing`;
  ...
  function addItem(event: React.FormEvent) {
    ...
    setItems((current) => [...current, { id: makeId(), label: label.trim(), category: category.trim() || "Other", packed: false }]);
    setLabel("");
  }
  ...
  return (
    <main className="archive-page workspace-page packing-page">
      <header className="archive-page-header">
        ...
        <div className="archive-header-links"><Link href={`/trips/${slug}/map`}>Journey map</Link><Link href="/trips">Change trip</Link></div>
      </header>
      <section className="archive-page-hero compact-archive-hero">
        <p className="eyebrow dark">TRIP PACKING · {slug.replaceAll("-", " ").toUpperCase()}</p>
        <h1>Ready before<br />the road begins.</h1>
        <p>This checklist is isolated from every other journey and saved automatically in this browser.</p>
      </section>
      <section className="packing-overview">
        <div className="packing-progress-copy"><p className="eyebrow dark">CURRENT JOURNEY</p><h2>{packedCount} of {items.length} packed</h2><p>{progress === 100 ? "Everything is ready for departure." : `${items.length - packedCount} items still need attention.`}</p></div>
        <div className="packing-progress" aria-label={`${progress}% packed`}><div className="packing-progress-ring" style={{ "--progress": `${progress * 3.6}deg` } as React.CSSProperties}><strong>{progress}%</strong></div><button type="button" onClick={() => setItems(defaultItems.map((item) => ({ ...item })))}><RotateCcw size={16} /> Reset checklist</button></div>
      </section>
      <section className="packing-workspace">
        <form className="packing-add-form" onSubmit={addItem}>
          ...
          <small>Saved under this trip: {slug}</small>
        </form>
        <div className="packing-lists">
          ...
          {categories.map((categoryName) => {
            ...
            return <section className={complete ? "packing-category complete" : "packing-category"} key={categoryName}><header><h3>{categoryName}</h3><span>{categoryItems.filter((item) => item.packed).length}/{categoryItems.length}</span></header>{categoryItems.map((item) => <div className={item.packed ? "packing-item packed" : "packing-item"} key={item.id}><button type="button" className="packing-toggle" onClick={() => setItems((current) => current.map((entry) => entry.id === item.id ? { ...entry, packed: !entry.packed } : entry))}>{item.packed ? <CheckCircle2 size={21} /> : <Circle size={21} />}<span>{item.label}</span></button><button type="button" className="packing-delete" onClick={() => setItems((current) => current.filter((entry) => entry.id !== item.id))} aria-label={`Delete ${item.label}`}><Trash2 size={16} /></button></div>)}</section>;
          })}
        </div>
      </section>
    </main>
