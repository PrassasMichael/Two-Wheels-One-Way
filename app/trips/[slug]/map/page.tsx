...
export default function TripMapPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(saved)) setStops(saved);
    } catch {}
    setLoaded(true);
  }, [storageKey]);

  useEffect(() => {
    if (loaded) localStorage.setItem(storageKey, JSON.stringify(stops));
  }, [loaded, stops, storageKey]);

  function addStop(event: FormEvent) {
    event.preventDefault();
    if (!name.trim()) return;
    setStops((current) => [...current, { id: crypto.randomUUID(), name: name.trim(), notes: notes.trim() }]);
    setName("");
    setNotes("");
  }
