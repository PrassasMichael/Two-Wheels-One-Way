...
function localSuggestions(payload: AdvisorPayload): Suggestion[] {
  ...
  if (route.length < 2) suggestions.push({
    priority: "important", category: "route", title: "Build a usable route",
    detail: "The journey has fewer than two saved stops, so distance, overnight rhythm and daily riding load cannot be assessed yet.",
    action: "Add the departure point and final destination to the live route.",
    change: { module: "route", mode: "append", label: "Add route foundations", items: [
      { name: String(trip.origin || "Departure point"), notes: "Journey starting point" },
      { name: String(trip.destination || "Final destination"), notes: "Journey destination" },
    ] },
  });
  if (route.length > 8) suggestions.push({
    priority: "recommended", category: "route", title: "Add a lighter riding day",
    detail: `You have ${route.length} route stops. A recovery or flexible day can protect the trip from fatigue and weather delays.`,
    action: "Add a flexible rest day to the itinerary.",
    change: { module: "route", mode: "append", label: "Add flexible rest day", items: [{ name: "Flexible rest day", notes: "Recovery, maintenance or weather buffer. Place this at the most demanding part of the route." }] },
  });

  const unpacked = packing.filter((item) => !item.packed);
  if (!packing.length) suggestions.push({
    priority: "important", category: "packing", title: "Start the motorcycle packing list",
    detail: "No packing data is saved for this trip yet.", action: "Import a practical starter checklist.",
    change: { module: "packing", mode: "append", label: "Import essential motorcycle gear", items: [
      { label: "Waterproof riding layers", category: "Travel gear", packed: false },
      { label: "Tyre repair kit and compact inflator", category: "Motorcycle", packed: false },
      { label: "First-aid kit and medication", category: "Health", packed: false },
      { label: "Chargers, cables and power bank", category: "Technology", packed: false },
    ] },
  });
  ...
  if (!budget.length) suggestions.push({
    priority: "recommended", category: "budget", title: "Create a basic trip budget",
    detail: "There are no planned costs, so fuel, accommodation, ferries, tolls and emergency margin are not visible.",
    action: "Import the essential budget categories and enter your estimates.",
    change: { module: "budget", mode: "append", label: "Import budget structure", items: [
      { title: "Fuel", category: "Fuel", planned: 0, actual: 0 },
      { title: "Accommodation", category: "Accommodation", planned: 0, actual: 0 },
      { title: "Ferries and tolls", category: "Transport", planned: 0, actual: 0 },
      { title: "Emergency reserve", category: "Other", planned: 0, actual: 0 },
    ] },
  });
        ...
        text: { format: { type: "json_schema", name: "trip_advice", strict: true, schema: {
          type: "object", additionalProperties: false, required: ["suggestions"], properties: { suggestions: {
            type: "array", maxItems: 8, items: { type: "object", additionalProperties: false,
              required: ["priority", "category", "title", "detail", "action", "change"],
              properties: {
                ...
                change: { anyOf: [
                  { type: "null" },
                  { type: "object", additionalProperties: false, required: ["module", "mode", "label", "items"], properties: {
                    module: { type: "string", enum: ["route", "packing", "budget", "documents", "journal"] },
                    mode: { type: "string", enum: ["append", "replace"] }, label: { type: "string" },
                    items: { type: "array", items: { type: "object", additionalProperties: { anyOf: [{ type: "string" }, { type: "number" }, { type: "boolean" }] } } }
                  } }
                ] }
              }
            }
